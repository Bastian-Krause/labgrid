...
var PTY_pollScratch = 0;

var PTY_wrapPoll = (impl, timeoutMs) => PTY_handleSleep(wakeUp => {
 const deadline = (timeoutMs === undefined || timeoutMs < 0) ? Infinity : Date.now() + timeoutMs;
 for (;;) {
  let result = impl();
  if (result === -1006) {
  ...
  if (result !== 0 || timeoutMs === 0 || typeof ENVIRONMENT_IS_PTHREAD === "undefined" || !ENVIRONMENT_IS_PTHREAD || Date.now() >= deadline) {
   wakeUp(result);
   return;
  }
  if (!PTY_pollScratch) PTY_pollScratch = _malloc(4) >> 2;
  Atomics.store(HEAP32, PTY_pollScratch, 0);
  Atomics.wait(HEAP32, PTY_pollScratch, 0, Math.min(4, deadline - Date.now()));
 }
});
...
var ___syscall_poll = (fds, nfds, timeout) => {
 ...
 for (let i = 0; i < nfds; i++) {
  const fd = HEAP32[(((fds) + (8 * i)) >>> 2) >>> 0];
  if (fd >= 0 && fd <= 2) {
   hasTty = true;
   break;
  }
 }
 return PTY_wrapPoll(() => xterm_pty_old_poll(fds, nfds, timeout), hasTty ? 0 : timeout);
};
...
var Asyncify = {
 ...
 instrumentWasmExports(exports) {
  for (var seedName of Object.keys(exports).sort()) {
   if (typeof exports[seedName] == "function") {
    Asyncify.getCallStackId(seedName);
   }
  }
  var ret = {};
 ...
 getDataRewindFunc(ptr) {
  ...
  if (typeof func !== "function") {
   err(`unresolvable asyncify rewind id ${id} (name ${name}, ${Object.keys(Asyncify.callStackNameToId).length} known)`);
  }
  return func;
 },
 doRewind(ptr) {
