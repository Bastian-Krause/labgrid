...
class AgentWrapper:
    def __init__(self, host=None):
        ...
        agent = os.path.join(os.path.abspath(os.path.dirname(__file__)), "agent.py")
        if host:
            ...
            ssh_opts = (
                f"ssh -x -o ConnectTimeout={connect_timeout} -o PasswordAuthentication=no".split()
            )
            subprocess.check_call(
                ["rsync", "-e", " ".join(ssh_opts), "-tq", agent, f"{host}:{agent_remote}"],
            )
