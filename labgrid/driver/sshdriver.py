...
class SSHDriver(CommandMixin, Driver, CommandProtocol, FileTransferProtocol):
    ...
    connection_timeout = attr.ib(
        default=float(get_ssh_connect_timeout()), validator=attr.validators.instance_of(float)
    )
    ...
    def _start_own_master_once(self, timeout):
        ...
        control = os.path.join(self.tmpdir, f"control-{self.networkservice.address}")
        ...
        proxy_cmd = proxymanager.get_command(
            self.networkservice, address, self.networkservice.port, ifname
        )
        if proxy_cmd:  # only proxy if needed
            args += ["-o", f"ProxyCommand={' '.join(proxy_cmd)} 2>{self.tmpdir}/proxy-stderr"]
        ...
        try:
            ...
            if return_value != 0:
                ...
                for line in stdout:
                    self.logger.warning(
                        "ssh: %s", line.rstrip().decode(encoding="utf-8", errors="replace")
                    )
        ...
        if not os.path.exists(control):
            raise ExecutionError(f"no control socket to {self.networkservice.address}")
    ...
    def _run(self, cmd, codec="utf-8", decodeerrors="strict", timeout=None):
        ...
        try:
            sub = subprocess.Popen(complete_cmd, stdout=subprocess.PIPE, stderr=stderr_pipe)
        except:
    ...
    def interact(self, cmd=None):
        ...
        complete_cmd = ["ssh", "-x", *self.ssh_prefix, "-t", self.networkservice.address]
        if cmd:
            complete_cmd += ["--", *cmd]
    ...
    def _forward(self, forward):
        cmd = ["ssh", *self.ssh_prefix, "-O", "forward", forward, self.networkservice.address]
        self.logger.debug("Running command: %s", cmd)
        subprocess.run(cmd, check=True)
        try:
            yield
        finally:
            cmd = ["ssh", *self.ssh_prefix, "-O", "cancel", forward, self.networkservice.address]
            self.logger.debug("Running command: %s", cmd)
    ...
    def scp(self, *, src, dst):
        ...
        complete_cmd = [
            ...
            "none",
            "-o",
            f"ControlPath={self.control.replace('%', '%%')}",
            src,
            dst,
        ]
    ...
    def put(self, filename, remotepath=""):
        ...
        try:
            ...
            raise ExecutionError(f"error executing command: {transfer_cmd}")
        if sub != 0:
            raise ExecutionError(f"error executing command: {transfer_cmd}")
    ...
    def _cleanup_own_master(self):
        ...
        complete_cmd = f"ssh -x -o ControlPath={self.control.replace('%', '%%')} -O exit -p {self.networkservice.port} -l {self.networkservice.username} {self.networkservice.address}".split(
            " "
