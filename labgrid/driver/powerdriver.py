...
class ManualPowerDriver(Driver, PowerResetMixin, PowerProtocol):
    ...
    @step()
    def on(self):
        self.target.interact(f"Turn the target {self.target.name} ON and press enter")

    @Driver.check_active
    @step()
    def off(self):
        self.target.interact(f"Turn the target {self.target.name} OFF and press enter")

    @Driver.check_active
    @step()
    def cycle(self):
        self.target.interact(f"CYCLE the target {self.target.name} and press enter")
...
class SiSPMPowerDriver(Driver, PowerResetMixin, PowerProtocol):
    ...
    bindings = {
        "port": {"SiSPMPowerPort", "NetworkSiSPMPowerPort"},
    }
    ...
    def _get_sispmctl_prefix(self):
        return self.port.command_prefix + [
            self.tool,
            "-U",
            f"{self.port.busnum:03d}:{self.port.devnum:03d}",
        ]
...
class NetworkPowerDriver(Driver, PowerResetMixin, PowerProtocol):
    ...
    def __attrs_post_init__(self):
        ...
        self.backend = import_module(f".power.{self.port.model}", __package__)
        self._host = None
        self._port = None
...
class USBPowerDriver(Driver, PowerResetMixin, PowerProtocol):
    ...
    bindings = {
        "hub": {"USBPowerPort", "NetworkUSBPowerPort"},
    }
    delay = attr.ib(default=2.0, validator=attr.validators.instance_of(float))
