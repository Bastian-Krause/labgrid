...
class ResourceExport(ResourceEntry):
    ...
    def acquire(self, *args, **kwargs):
        if self.broken:
            raise BrokenResourceError(
                f"cannot acquire broken resource (original reason): {self.broken}"
            )
    ...
    def release(self, *args, **kwargs):
        if self.broken:
            raise BrokenResourceError(
                f"cannot release broken resource (original reason): {self.broken}"
            )
...
class ExporterSession(ApplicationSession):
    ...
    async def add_resource(self, group_name, resource_name, cls, params):
        ...
        if issubclass(export_cls, ResourceExport):
            group[resource_name] = export_cls(
...
def main():
    ...
    parser.add_argument(
        "resources", metavar="RESOURCES", type=str, help="resource config file name"
    )
