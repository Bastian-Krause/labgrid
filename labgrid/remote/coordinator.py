        ...
        for session in list(self.sessions.values()):
            if isinstance(session, ExporterSession):
                ...
                if not done:
                    ...
                    await self.call(
                        "wamp.session.kill", session.key, message="timeout detected by coordinator"
                    )
    ...
    async def _acquire_resources(self, place, resources):
        ...
        try:
            for resource in resources:
                ...
                # to the clients
                await self.call(
                    f"org.labgrid.exporter.{resource.path[0]}.acquire",
                    resource.path[1],
                    resource.path[3],
                    place.name,
                )
    ...
    async def _release_resources(self, place, resources, callback=True):
        ...
        for resource in resources:
            try:
                ...
                if callback:
                    await self.call(
                        f"org.labgrid.exporter.{resource.path[0]}.release",
                        resource.path[1],
                        resource.path[3],
                    )
            except:
