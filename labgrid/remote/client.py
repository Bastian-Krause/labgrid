...
class ClientSession(ApplicationSession):
    ...
    def onChallenge(self, challenge):
        ...
        warnings.warn(
            "Ticket authentication is deprecated. Please update your coordinator.",
            DeprecationWarning,
        )
    ...
    async def on_resource_changed(self, exporter, group_name, resource_name, resource):
        ...
        if self.monitor:
            if resource and not old:
                print(
                    f"Resource {exporter}/{group_name}/{resource['cls']}/{resource_name} created: {resource}"
                )
            elif resource and old:
                print(
                    f"Resource {exporter}/{group_name}/{resource['cls']}/{resource_name} changed:"
                )
    ...
    async def print_resources(self):
            ...
            for group_name, group in sorted(groups.items()):
                for resource_name, resource in sorted(group.items()):
                    ...
                    if self.args.acquired and resource.acquired is None:
                        continue
                    if match and not match.ismatch(
                        (exporter, group_name, resource.cls, resource_name)
                    ):
                        continue
        ...
        if self.args.verbose and not self.args.sort_by_matched_place_change:
            ...
            for places, exporter, group_name, resource_cls in results:
                if self.args.sort_by_matched_place_change:
                    places_strs = [
                        f"{p.name}: {datetime.fromtimestamp(p.changed):%Y-%m-%d}" for p in places
                    ]
    ...
    async def print_place(self):
        ...
        if place.acquired:
            for resource_path in place.acquired_resources:
                ...
                print(
                    f"Acquired resource '{name}' ({exporter}/{group_name}/{resource.cls}/{resource_name}):"
                ...
                for group_name, group in sorted(groups.items()):
                    for resource_name, resource in sorted(group.items()):
                        ...
                        if match.rename:
                            name = match.rename
                        print(
                            f"Matching resource '{name}' ({exporter}/{group_name}/{resource.cls}/{resource_name}):"
                        )  # pylint: disable=line-too-long
                        print(indent(pformat(resource.asdict()), prefix="  "))
            ...
            if name is not None:
                ...
                try:
                    ...
                    raise NotImplementedError(
                        "Multiple bindings not implemented for named resources"
                    )
    ...
    def power(self):
        ...
        if action == "get":
            print(
                f"power{' ' + name if name else ''} for place {place.name} is {'on' if res else 'off'}"
            )
    ...
    def digital_io(self):
        ...
        if action == "get":
            print(
                f"digital IO{' ' + name if name else ''} for place {place.name} is {'high' if drv.get() else 'low'}"
            )
        elif action == "high":
            drv.set(True)
        elif action == "low":
    ...
    async def _console(
        self, place, target, timeout, *, logfile=None, loop=False, listen_only=False
    ):
        name = self.args.name
    ...
    async def console(self, place, target):
        while True:
            res = await self._console(
                ...
                loop=self.args.loop,
                listen_only=self.args.listenonly,
            )
            if res:
    ...
    async def print_reservations(self):
        ...
        for token, config in sorted(
            reservations.items(), key=lambda x: (-x[1]["prio"], x[1]["created"])
        ):  # pylint: disable=line-too-long
            config = filter_dict(config, Reservation, warn=True)
            res = Reservation(token=token, **config)
            print(f"Reservation '{res.token}':")
...
def main():
    ...
    parser.add_argument(
        "-c", "--config", type=str, default=os.environ.get("LG_ENV"), help="config file"
    )
    ...
    subparser.add_argument("-a", "--acquired", action="store_true")
    subparser.add_argument(
        "--sort-last-changed", action="store_true", help="sort by last changed date (oldest first)"
    )
    ...
    subparser.add_argument(
        "tags", metavar="KEY=VALUE", nargs="+", help="use an empty value for deletion"
    )
    subparser.set_defaults(func=ClientSession.set_tags)

    subparser = subparsers.add_parser(
        "add-match", help="add one (or multiple) match pattern(s) to a place"
    )
    subparser.add_argument("patterns", metavar="PATTERN", nargs="+")
    subparser.set_defaults(func=ClientSession.add_match)

    subparser = subparsers.add_parser(
        "del-match", help="delete one (or multiple) match pattern(s) from a place"
    )
    subparser.add_argument("patterns", metavar="PATTERN", nargs="+")
    subparser.set_defaults(func=ClientSession.del_match)

    subparser = subparsers.add_parser(
        "add-named-match", help="add one match pattern with a name to a place"
    )
    ...
    subparser.add_argument(
        "acquired", metavar="HOST/USER", help="User and host to match against when releasing"
    )
    ...
    subparser = subparsers.add_parser(
        "power", aliases=("pw",), help="change (or get) a place's power status"
    )
    ...
    subparser.add_argument(
        "altsetting", help="altsetting name or number (download, detach only)", nargs="?"
    )
    ...
    subparser.add_argument(
        "fastboot_args", metavar="ARG", nargs=argparse.REMAINDER, help="fastboot arguments"
    )
    subparser.add_argument("--wait", type=float, default=10.0)
    subparser.add_argument("--name", "-n", help="optional resource name")
    ...
    subparser.add_argument(
        "script_args", metavar="ARG", nargs=argparse.REMAINDER, help="script arguments"
    )
    subparser.add_argument("--name", "-n", help="optional resource name")
    ...
    subparser.add_argument(
        "bootstrap_args", metavar="ARG", nargs=argparse.REMAINDER, help="extra bootstrap arguments"
    )
    subparser.add_argument("--name", "-n", help="optional resource name")
    ...
    subparser.add_argument(
        "links", choices=["off", "dut-device", "host-dut", "host-device", "host-dut+host-device"]
    )
    subparser.add_argument("--name", "-n", help="optional resource name")
    ...
    subparser.add_argument(
        "-q", "--quality", type=str, help="select a video quality (use 'list' to show options)"
    )
    subparser.add_argument(
        "-c",
        "--controls",
    ...
    subparser.add_argument(
        "--skip", type=int, default=0, help="skip n 512-sized blocks at start of input"
    )
    subparser.add_argument(
        "--seek", type=int, default=0, help="skip n 512-sized blocks at start of output"
    )
    subparser.add_argument(
        "--mode",
        dest="write_mode",
    ...
    subparser.add_argument(
        "--wait", action="store_true", help="wait until the reservation is allocated"
    )
    ...
    if args.command != "reserve" and env and env.config.get_targets():
        if args.place:
            if not args.place.startswith("+"):
                ...
                if not role:
                    print(
                        f"RemotePlace {args.place} not found in configuration file", file=sys.stderr
                    )
                    exit(1)
