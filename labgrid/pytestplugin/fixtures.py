...
def pytest_addoption(parser):
    ...
    group.addoption(
        "--lg-env", action="store", dest="lg_env", help="labgrid environment config file."
    )
    group.addoption(
        "--lg-coordinator",
        action="store",
    ...
    parser.addini(
        ...
        help="Default value for log_format (overwritten by labgrid)",
    )
...
def env(request, record_testsuite_property):
    ...
    for target_name in targets:
        ...
        try:
            ...
            record_testsuite_property(f"TARGET_{target_name.upper()}_REMOTE", remote_name)
    ...
    for name, path in env.config.get_paths().items():
        ...
        record_testsuite_property(
            f"PATH_{name.upper()}_GIT_COMMIT", sha.decode("utf-8").strip("\n")
        )

    for name, image in env.config.get_images().items():
        record_testsuite_property(f"IMAGE_{name.upper()}", image)
        try:
            sha = subprocess.check_output(
                "git rev-parse HEAD".split(), cwd=os.path.dirname(image), stderr=subprocess.DEVNULL
            )
        except subprocess.CalledProcessError:
            continue
        except FileNotFoundError:
            continue
        record_testsuite_property(
            f"IMAGE_{name.upper()}_GIT_COMMIT", sha.decode("utf-8").strip("\n")
        )
