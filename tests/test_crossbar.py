...
def test_connect_error():
    with pexpect.spawn(
        "python -m labgrid.remote.client -x ws://127.0.0.1:20409/ws places"
    ) as spawn:
...
def test_place_match(place):
    with pexpect.spawn(
        'python -m labgrid.remote.client -p test add-match "e1/g1/r1" "e2/g2/*"'
    ) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_match_duplicates(place):
    ...
    for match in matches:
        with pexpect.spawn(
            f'python -m labgrid.remote.client -p test add-match "{match[0]}" "{match[1]}"'
        ) as spawn:
        ...
        with pexpect.spawn(
            f'python -m labgrid.remote.client -p test del-match "{match[0]}"'
        ) as spawn:
            spawn.expect(pexpect.EOF)
            spawn.close()
            assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_place_acquire_enforce(place):
    ...
    with pexpect.spawn(
        "python -m labgrid.remote.client -p test acquire --allow-unmatched"
    ) as spawn:
...
def test_place_release_from(monkeypatch, place, exporter):
    ...
    with pexpect.spawn(
        f"python -m labgrid.remote.client -p test release-from {host}/{user}"
    ) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_resource_conflict(place_acquire, tmpdir):
    ...
    with pexpect.spawn(
        'python -m labgrid.remote.client -p test2 add-match "*/Testport/*"'
    ) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_reservation(place_acquire, tmpdir):
    with pexpect.spawn(
        "python -m labgrid.remote.client reserve --shell board=bar name=test"
    ) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
        m = re.search(
            rb"^export LG_TOKEN=(\S+)$", spawn.before.replace(b"\r\n", b"\n"), re.MULTILINE
        )
...
def test_reservation_custom_config(place, exporter, tmpdir):
    ...
    with pexpect.spawn(
        f"python -m labgrid.remote.client -c {p} reserve --wait --shell board=bar name=test"
    ) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
...
def test_same_name_resources(place, exporter, tmpdir):
    with pexpect.spawn(
        'python -m labgrid.remote.client -p test add-named-match "testhost/Many/NetworkService" "samename"'
    ) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn(
        'python -m labgrid.remote.client -p test add-named-match "testhost/Many/NetworkSerialPort" "samename"'
    ) as spawn:
        spawn.expect(pexpect.EOF)
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
    ...
    with pexpect.spawn("python -m labgrid.remote.client -p test env") as spawn:
        ...
        spawn.close()
        assert spawn.exitstatus == 0, spawn.before.strip()
        assert "NetworkService".encode("utf-8") in spawn.before.replace(
            b"\r\n", b"\n"
        ), spawn.before.strip()
        assert "NetworkSerialPort".encode("utf-8") in spawn.before.replace(
            b"\r\n", b"\n"
        ), spawn.before.strip()
