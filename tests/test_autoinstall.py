...
def test_autoinstall_help():
    with lg_spawn("python -m labgrid.autoinstall.main --help") as spawn:
        spawn.expect("usage")
...
def test_autoinstall_error_missing_autoinstall(tmpdir):
    ...
    targets:
        test: {}
    """)
    with lg_spawn(f"python -m labgrid.autoinstall.main {c}") as spawn:
...
def test_autoinstall_simple(tmpdir):
    ...
    autoinstall:
        handler: |
            print("handler-test-output")
    """)
    with lg_spawn(f"python -m labgrid.autoinstall.main --once {c}") as spawn:
