...
pytestmark = pytest.mark.skipif(not which("sigrok-cli"), reason="sigrok not available")


def test_sigrok_resource(target):
    r = SigrokUSBDevice(
        target, name=None, match={"sys_name": "1-12"}, driver="fx2lafw", channels="D0,D1"
    )
...
def test_sigrok_usb_driver(target, tmpdir):
    r = SigrokUSBDevice(
        target,
        name=None,
        match={"ID_MODEL_ID": "3881", "ID_VENDOR_ID": "0925"},
        driver="fx2lafw",
        channels="D0,D1",
    )
    d = SigrokDriver(target, name=None)
